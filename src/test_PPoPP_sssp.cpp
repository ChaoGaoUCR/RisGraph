...
int main(int argc, char** argv)
{
    ...
    for (int round = 1; round < compute_batch+1; round++)
    {
        ...
        for (int small_round = 1; small_round < 2; small_round++)
        {
        Graph<uint64_t> core_graph(num_vertices, query_specific_core_graph.size(), false, true);
        uint64_t count = 0;
        fprintf(stderr,"*********************************************\n");
    ...
    // sampling smaller graph
            ...
            for (uint64_t src = 0; src < num_vertices; src++)
            {
                ...
                for (uint64_t j = 0; j < degreeV; j++)
                {
                    ...
                    uint64_t edge_len = (src+dst)%128+1;
                    if ((labels[src].data + edge_len == labels[dst].data) && (degreeV != 0))
                    {
                        for (uint64_t search_edge = seed; search_edge < seed+batch_current; search_edge++)
                        {
                            if ((raw_edges[search_edge].first == src) && (raw_edges[search_edge].second == dst))
                            {
                        ...
                        if (graph.check_edge({src,dst, edge_len}) == true)
                        {
                            query_specific_core_graph.insert(std::make_pair(src,dst));
                            outflag[src] = true;                            
                        }
                            ...
                            // wrong++;
                            // fprintf(stderr,"wrong edge (%ld, %ld)\n", src, dst);
                            }
                    }
                }
            ...
            // for (uint64_t src = 0; src < num_vertices; src++)
            // {
            //     if (outflag[src] == false)
            //     {
            //         if (graph.get_outgoing_degree(src)!=0)
            //         {
            //             uint64_t dst = graph.get_dst_number(src, 0);
            //             query_specific_core_graph.insert(std::make_pair(src, dst));
            //         }
            //     }
            // }
        ...
        for (auto e: query_specific_core_graph)
        {
            ...
            for (uint64_t i = 0; i < num_vertices; i++)
            {
                if (labels[i].data != core_query[i].data)
                {
                    count++;
                    fprintf(stderr,"Results correct is %ld, and wrong result is %ld\n", labels[i].data, core_query[i].data);

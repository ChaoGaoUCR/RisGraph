 ...
 */
     ...
     uint64_t& currentUpper)
 {
     ...
     {
             uint64_t lowerBound = targetToHot & hotToAnySrc;
             uint64_t upperBound = targetToHot | hotToAnySrc;
    ...
    THRESHOLD_OPENMP_LOCAL("omp parallel for", raw_edges_len, 1024,
       for (auto edge = 0; edge < raw_edges_len; edge++)
       {
           ...
           if (versionCheck(tag.first, tag.second, snapShotNum))
           {
               ...
               {
                   bool outFlag = ((outRankResult[idx][src].data != outRankResult[idx][dst].data) || ((outRankResult[idx][src].data == true) && (outRankResult[idx][dst].data == true))) && !outRankResultAtomic[idx][dst];
                   bool inFlag = (inRankResult[idx][dst].data != inRankResult[idx][src].data) || ((inRankResult[idx][dst].data == true) && (inRankResult[idx][src].data == true)) && !inRankResultAtomic[idx][src];
                   if (outFlag || inFlag)
                   {
                       #pragma omp critical
                       {

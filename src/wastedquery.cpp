...
int main(int argc, const char** argv) {
    ...
    auto results = rootCompute(graphBase, root);
    auto compareResults = rootCompute(graphBase, root);
    for (auto i = 0; i < 1; i++)
    {
        rootIncrementalCompute(graphBase, root, results, addition_batches[0], deletion_batches[0]);
        std::atomic<uint64_t> count(0);
        THRESHOLD_OPENMP_LOCAL("omp parallel for", batch_num, 1024,
            for(auto node = 0; node < graphBase.getNodesNum(); node++)
            {
                if (results[node].data != compareResults[node].data)
                {
                    count.fetch_add(1);
                }
            }
        );
        fprintf(stderr, "Wasted Percentage: %lf %%\n", 100.0 * count.load() / graphBase.getNodesNum());
    }
    return 0;
}
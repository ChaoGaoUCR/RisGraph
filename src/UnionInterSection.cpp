...
int main(int argc, char** argv)
{
    ...
    for (auto snapshotGraph = 0; snapshotGraph < 1; snapshotGraph++)
    {
        ...
        Graph<uint64_t> coreForCommonGraph(num_vertices, coreCommonGraphEdges.size(), false, true);
        Graph<uint64_t> coreForUnionGraph(num_vertices, coreForAllEdges.size(), false, true);
        ...
        for (uint64_t i = 0; i < coreCommonGraphEdges.size(); i++)
        {
            ...
            coreForCommonGraph.add_edge({e.first, e.second, (e.first+e.second)%16 + 1}, true);
        }
        );  
        THRESHOLD_OPENMP_LOCAL("omp parallel for", coreForAllEdges.size(), 1024,
        for (uint64_t i = 0; i < coreForAllEdges.size(); i++)
        {
            const auto &e = coreForAllEdges[i];
            coreForUnionGraph.add_edge({e.first, e.second, (e.first+e.second)%16 + 1}, true);
        }
        );  
        ...
        auto coreResultCommon = coreForCommonGraph.alloc_vertex_tree_array_vector<uint64_t>(srcToCalculate);    
        auto coreResultUnion = coreForUnionGraph.alloc_vertex_tree_array_vector<uint64_t>(srcToCalculate);
        for (auto i = 0; i < srcToCalculate; i++)
        {
            correctResult[i] = rootCompute(graph, i);
            coreResult[i] = rootCompute(coreForAllGraph, i);
            coreResultCommon[i] = rootCompute(coreForCommonGraph, i);
            coreResultUnion[i] = rootCompute(coreForUnionGraph, i);
        }
        ...
        std::atomic<uint64_t> correctCountCommon(0);
        std::atomic<uint64_t> wrongCountCommon(0);
        std::atomic<uint64_t> correctCountUnion(0);
        std::atomic<uint64_t> wrongCountUnion(0);
        for (uint64_t i = 0; i < srcToCalculate; i++)
        {
            ...
            for (uint64_t j = 0; j < graph.getNodesNum(); j++)
            {
                ...
                if (correctResult[i][j].data == coreResultCommon[i][j].data)
                {
                    correctCountCommon.fetch_add(1);
                }
                else
                {
                    if (correctResult[i][j].data > coreResultCommon[i][j].data)
                    {
                        wrongCountCommon.fetch_add(1);
                    }
                }
                if (correctResult[i][j].data == coreResultUnion[i][j].data)
                {
                    correctCountUnion.fetch_add(1);
                }
                else
                {
                    if (correctResult[i][j].data > coreResultUnion[i][j].data)
                    {
                        wrongCountUnion.fetch_add(1);
                    }   
                }
            }
            );
        ...
        fprintf(stderr," %d Snapshot has %.2f%% correct result\n", snapshotGraph, (100.0 * correctCountCommon.load()) / (num_vertices * srcToCalculate));
        fprintf(stderr," %d Snapshot has %.2f%% correct result\n", snapshotGraph, (100.0 * correctCountUnion.load()) / (num_vertices * srcToCalculate));

...
int main(int argc, char** argv)
{
    ...
    // WorkSharing None Mutation Incremental Addition Computation Happens here:
    // Each lvl involves two batches, first with bigger one contains add_(i) ... add_(batch_num - 1) and smaller one contains del_(i)
    // labels will be used for bigger batch while common_labels will be used for smaller batch
    // each round we will reset common_labels to labels
    // each computation will clean the batch index
    std::vector<double> execTime(batch_num -1);
    for(uint64_t round=0; round < batch_num; round++)
    {   
        double execTimeNoneMutation = 0;
        // Bigger Batch will contains add only
        {
            auto biggerStartIndex = round;
            auto biggerEndIndex = batch_num;
            std::vector<decltype(graph)::edge_type> addedEdgesNoneMutation((batch_num - round) *  batch_size);
            std::atomic_uint64_t lengthNoneMutation(0);
            std::vector<uint64_t> additionBatchIndex;
            for(uint64_t i = biggerStartIndex; i < biggerEndIndex; i++)
            {
                additionBatchIndex.push_back(i);
            }
            graph.batchCoverageUpdate(additionBatchIndex, {});
            #pragma omp parallel for
            for(uint64_t i = biggerStartIndex; i < biggerEndIndex; i++)
            {
                for(uint64_t j = 0; j < batch_size; j++)
                {
                    auto &e = addition_batches[i][j];
            ...
            graph.update_tree_add<uint64_t, uint64_t>(
                ...
                update_func,
                active_result_func,
                labels, addedEdgesNoneMutation, lengthNoneMutation, true 
            );
            auto end = std::chrono::system_clock::now();
            execTimeNoneMutation += 1e-6*(uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
            graph.clearBatchIndex();
        }
        // Smaller Batch Only contains deletion round
        {
            std::vector<decltype(graph)::edge_type> addedEdgesNoneMutation(batch_size);
            std::atomic_uint64_t lengthNoneMutation(0);
            std::vector<uint64_t> deletionBatchIndex;
            deletionBatchIndex.push_back(round);
            graph.batchCoverageUpdate({}, deletionBatchIndex);
            #pragma omp parallel for
            for(uint64_t i=0; i<batch_size; i++)
            {
                auto &e = deletion_batches[round][i];
                addedEdgesNoneMutation[lengthNoneMutation.fetch_add(1)] = {e.first, e.second, (e.first+e.second)%16 + 1};
            }
            auto start = std::chrono::system_clock::now();
            graph.update_tree_add<uint64_t, uint64_t>(
        ...
        }
        {
            ...
            {
                labels[node].data = common_labels[node].data;
            }
        }
        execTime[round] = execTimeNoneMutation;
    }

        ...
        //add edges and doing addition computation        
    {
            ...
            if (small_round == 1)
            {
                ...
                for (uint64_t tracker = 0; tracker < num_vertices; tracker++)
                {
                    ...
                    track_before[tracker] =  graph.nodes_track[tracker];
                    ratio[small_round-1] = static_cast<_Float64>(edge_similar)/edge_traverse;                
                }
            }
            graph.clear_track();
        ...
        // auto max_s = std::min_element(ratio.begin(), ratio.begin());
        // fprintf(stderr, "The min similarity is %f %\n", (*max_s)*100);
        for ( auto i:ratio)
        {

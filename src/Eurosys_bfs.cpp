...
int main(int argc, char** argv)
{
    ...
    std::atomic_uint64_t length_add(0);        
    {
        ...
        {
            auto &e = raw_edges[i];
            del_edge[length_del.fetch_add(1)] = {e.first, e.second, (e.first+e.second)%1+1};
            auto &e1 = raw_edges[i+big_batch];
            add_edge[length_add.fetch_add(1)] = {e1.first, e1.second, (e1.first+e1.second)%1+1};
        }
    }
    // fprintf(stderr,"sampling edges generated finished sample seed is %ld\n", seed);
    {
        ...
        {
            // const auto &e = raw_edges[i];
            graph.add_edge({raw_edges[i].first, raw_edges[i].second, (raw_edges[i].first+raw_edges[i].second)%1+1}, true);
        }
        // common graph read
        #pragma omp parallel for
        ...
        {
            // const auto &e = raw_edges[i];
            common_graph.add_edge({raw_edges[i].first, raw_edges[i].second, (raw_edges[i].first+raw_edges[i].second)%1+1}, true);
            // core_graph.add_edge({raw_edges[i].first, raw_edges[i].second, (raw_edges[i].first+raw_edges[i].second)%1+1}, true);
        }
        #pragma omp parallel for
        ...
        {
            // const auto &e = raw_edges[i];
            snapshot_graph.add_edge({raw_edges[i].first, raw_edges[i].second, (raw_edges[i].first+raw_edges[i].second)%1+1}, true);
        }
        #pragma omp parallel for
    ...
    // #pragma omp parallel for
    // for (uint64_t vid = 0; vid < match; vid++)
    // {
    //     uint64_t vertex = record[vid];
    //     uint64_t in_coming_ptr = core_graph.get_incoming_degree(vertex);
    //     #pragma omp parallel for
    //     for (uint64_t ptr = 0; ptr < in_coming_ptr; ptr++)
    //     {
    //         uint64_t src = core_graph.get_incoming_adjlist(vertex)[ptr].nbr;
    //         uint64_t len = (src + vertex) % 1 + 1;
    //         core_graph.del_edge({src, vertex, len}, true);
    //     }
    ...
    //         tmp_add_edge[tmp_length_add.fetch_add(1)] = {e.first, e.second, (e.first+e.second)%1+1};
    //         tmp_del_edge[tmp_length_del.fetch_add(1)] = {e.first, e.second, (e.first+e.second)%1+1};
    //     }
    ...
    //             auto old_num = graph.del_edge({e.first, e.second, (e.first+e.second)%1+1}, true);
    //         }
    //     );
    ...
    //         {
    //             const auto &e = raw_edges[i+seed];
    //             auto old_num = graph.add_edge({e.first, e.second, (e.first+e.second)%1+1}, true);
    //         }
    //     );
    ...
    //         {
    //             const auto &e = raw_edges[i+seed];
    //             auto old_num = core_graph.add_edge({e.first, e.second, (e.first+e.second)%1+1}, true);
    //         }
    //     );
    ...
    //         {
    //             const auto &e = raw_edges[i+seed];
    //             auto old_num = core_graph.del_edge({e.first, e.second, (e.first+e.second)%1+1}, true);
    //         }
    //     );

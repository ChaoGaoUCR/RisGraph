...
int main(int argc, char** argv)
{
    ...
    std::atomic_uint64_t lengthNoneMutation(0);
    {
        ...
        for(uint64_t i=0; i<batch_num; i++)
        {
            ...
            {
                auto &e = deletion_batches[i][j];
                addedEdgesNoneMutation[lengthNoneMutation.fetch_add(1)] = {e.first, e.second, (e.first+e.second)%16 + 1};
            }
        }
        fprintf(stderr,"%lu edges are added in None Mutation\n", lengthNoneMutation.load());
    }
    {
        auto start = std::chrono::system_clock::now();
        ...
        auto end = std::chrono::system_clock::now();
        fprintf(stderr, "None Mutation incremental exec: %.6lfs\n", 1e-6*(uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(end-start).count());
    }    
    ...
    std::vector<decltype(graph)::edge_type> added_edges(batch_num *  batch_size), deled_edges(batch_num * batch_size);
    added_edges.clear(); deled_edges.clear();    
    {
        ...
        auto end = std::chrono::system_clock::now();
        fprintf(stderr, "Snapshot 0 Added Time: %.6lfs\n", 1e-6*(uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(end-start).count());
    }
    uint64_t numSnapshot0Edges = graph.get_degree();
    fprintf(stderr, "Number of Edges in Snapshot 0 is %lu\n", numSnapshot0Edges);
    ...
    {
        fprintf(stderr, "Number of Edges in Snapshot 0 is correct\n");
    }
    
    {
        ...
        graph.update_tree_add<uint64_t, uint64_t>(
            ...
            update_func,
            active_result_func,
            labels, addedEdgesNoneMutation, batch_size * batch_num, true 
        );
        auto end = std::chrono::system_clock::now();
        fprintf(stderr, "Mutation incremental exec: %.6lfs\n", 1e-6*(uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(end-start).count());
    }  

    auto test_labels = graph.alloc_vertex_tree_array<uint64_t>();

    {
        ...
        auto end = std::chrono::system_clock::now();
        fprintf(stderr, "Test exec: %.6lfs\n", 1e-6*(uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(end-start).count());
    }

    //Compare Results between test_labels and common_labels
    {
        bool match = true;
        uint64_t count = 0;
        for(uint64_t node = 0; node < num_vertices; node ++){
            if(common_labels[node].data != test_labels[node].data)
            {
                match = false;
                count++;
                // fprintf(stderr,"%lu nodes result is %lu in test but %lu in other\n", node, test_labels[node].data,
                // common_labels[node].data);
            }
        }
        if(match)
        {
            fprintf(stderr,"None Mutation result match\n");
        }
        else
        {
            fprintf(stderr,"%lu nodes result is not match\n", count);
            fprintf(stderr,"None Mutation not match\n");
        }
    }
    {
        bool match = true;
        uint64_t count = 0;
        for(uint64_t node = 0; node < num_vertices; node ++){
            if(labels[node].data != test_labels[node].data)
            {
                match = false;
                count++;
                // fprintf(stderr,"%lu nodes result is %lu in test but %lu in other\n", node, test_labels[node].data,
        ...
        if(match)
        {
            fprintf(stderr,"Mutation result match\n");
        }
        else
        {
            fprintf(stderr,"%lu nodes result is not match\n", count);
            fprintf(stderr,"Mutation Result not match\n");
        }
    }    
    return 0;
}

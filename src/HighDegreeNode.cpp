...
std::vector<uint64_t> rank_in(Graph<uint64_t>& G, uint64_t num = 20)
{
...
std::vector<uint64_t> rank_out(Graph<uint64_t>& G, uint64_t num = 20)
{
    std::vector<uint64_t> rank;
    rank.reserve(num);
    std::vector<std::pair<uint64_t, uint64_t>> vertex_;
                                                                ...
                                                                uint64_t num = 20)
{
    ...
    // choose 20 High Degree Nodes for core graph generation
    auto rankIn = rank_in(graph, num);  
    auto rankOut = rank_out(graph, num);
    auto outRankResult = graph.alloc_vertex_tree_array_vector<uint64_t>(rankOut.size());
    auto inRankResult = graph.alloc_vertex_tree_array_vector<uint64_t>(rankIn.size());
...
int main(int argc, char** argv)
{
    if (argc != 7)
    {
        fprintf(stderr, "usage: %s graph root_file batch_num batch size high_degree_num projection\n", argv[0]);
        exit(1);
    }
    std::pair<uint64_t, uint64_t> *raw_edges = nullptr;
    std::vector<uint64_t> roots = readNumbersFromFile(argv[2]);
    uint64_t high_degree_num = std::stoull(argv[5]);
    uint64_t raw_edges_len;
    uint64_t projection = std::stoull(argv[6]);
    ...
    auto unionCoreGraphEdges = coreGenerateVector(graph, raw_edges, E_tag, raw_edges_len, 999, high_degree_num);
    ...
    auto coreCommonGraphEdges = coreGenerateVector(graph, raw_edges, E_tag, raw_edges_len, 666, high_degree_num);
    ...
    for (auto snapshotGraph = 0; snapshotGraph < 1; snapshotGraph++)
    {
        ...
        for (uint64_t i = 0; i < coreForAllEdges.size(); i++)
        {
            ...
            {
                throw std::runtime_error("Projection FLAG is not supported");
            }
        }
        );

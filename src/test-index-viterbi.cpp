    ...
    uint64_t& currentUpper)
{
    if (hotToAnySrc != MAXL && targetToHot != MAXL)
    {
            ...
            uint64_t upperBound = (hotToAnySrc > targetToHot) ? hotToAnySrc : targetToHot;
...
std::vector<std::pair<uint64_t, uint64_t>> MergeTwoVectorEdge(std::vector<std::pair<uint64_t, uint64_t>> &a, 
 ...
 int main(int argc, char** argv)
 {
     ...
     for (auto snapshotGraph = 0; snapshotGraph < testSnapshot; snapshotGraph++)
     {
         ...
         {
             highRankVerticesQuery[hotNode] = rootCompute(graph, hotNode);
         }
         // Get HighRank Vertices Queries In
         graph.transpose();
         auto highRankVerticesQueryIn = graph.alloc_vertex_tree_array_vector<uint64_t>(rankTriple.size());
         for (auto & hotNode : rankTriple)
         {
             highRankVerticesQueryIn[hotNode] = rootCompute(graph, hotNode);
         }
         graph.transpose();

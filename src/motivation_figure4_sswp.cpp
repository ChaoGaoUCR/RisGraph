...
auto rootCompute(Graph<uint64_t>& graph, uint64_t root) {
    ...
    const uint64_t MINL = 0;  // For widest path, we initialize with 0
    auto continue_reduce_func = [](uint64_t depth, uint64_t total_result, uint64_t local_result) -> std::pair<bool, uint64_t> {
        return std::make_pair(local_result > 0, std::max(total_result, local_result));
    };
    ...
    auto update_func = [](uint64_t src, uint64_t dst, uint64_t src_data, uint64_t dst_data, AdjEdgeType adjedge) -> std::pair<bool, uint64_t> {
        // For widest path, we take the minimum of the current path width and the edge weight
        uint64_t new_width = std::min(src_data, adjedge.data);
        return std::make_pair(new_width > dst_data, new_width);
    };
    ...
    auto equal_func = [](uint64_t src, uint64_t dst, uint64_t src_data, uint64_t dst_data, AdjEdgeType adjedge) -> bool {
        uint64_t new_width = std::min(src_data, adjedge.data);
        return new_width == dst_data;
    };

    auto init_label_func = [=](uint64_t vid) -> std::pair<uint64_t, bool> {
        return {vid == root ? UINT64_MAX : MINL, vid == root};  // Initialize root with maximum value
    };
...
std::vector<std::pair<uint64_t, uint64_t>> core_generate(Graph<uint64_t>& graph)
{
    ...
    for (uint64_t i = 0; i < graph.getNodesNum(); i++) {
        ...
        edge_flag[i].resize(outDegree, false);
    }
    ...
    for (uint64_t edgeIndex = 0; edgeIndex < totalEdges; edgeIndex++) {
        ...
        for (uint64_t idx = 0; idx < rankOut.size(); idx++) {
            if (outRankResult[idx][src].data == outRankResult[idx][dst].data) {
    ...
    for (uint64_t edgeIndex = 0; edgeIndex < totalEdges; edgeIndex++) {
        ...
        for (uint64_t idx = 0; idx < rankIn.size(); idx++) {
            if (inRankResult[idx][src].data == inRankResult[idx][dst].data) {
    ...
    THRESHOLD_OPENMP_LOCAL("omp parallel for", raw_edges_len, 1024,
        for (auto edge = 0; edge < raw_edges_len; edge++)
        {
            ...
            if (versionCheck(tag.first, tag.second, snapShotNum))
            {
                ...
                for (uint64_t idx = 0; idx < rankOut.size(); idx++) 
                {
                    auto outRankSource = outRankResult[idx][src].data;
                    auto inRankSource = inRankResult[idx][dst].data;
...
int main(int argc, char** argv)
{
    ...
    Graph<uint64_t> unionGraph(num_vertices, unionCoreGraphEdges.size(), false, true);
    {
        auto start = std::chrono::system_clock::now();
        THRESHOLD_OPENMP_LOCAL("omp parallel for", unionCoreGraphEdges.size(), 1024,
            for(uint64_t i = 0; i < unionCoreGraphEdges.size(); i++) {
                const auto &e = unionCoreGraphEdges[i];
                unionGraph.add_edge({e.first, e.second, (e.first + e.second) % 16 + 1}, true);
            }
        );
        auto end = std::chrono::system_clock::now();
        fprintf(stderr, "Union Graph Initialization: %.6lfs\n", 1e-6*(uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(end-start).count());
    }
    fprintf(stderr, "Number of Edges in Union Graph is %lu\n", unionGraph.get_degree());

    Graph<uint64_t> commonGraph(num_vertices, coreCommonGraphEdges.size(), false, true);
    {
        auto start = std::chrono::system_clock::now();
        THRESHOLD_OPENMP_LOCAL("omp parallel for", coreCommonGraphEdges.size(), 1024,
            for(uint64_t i = 0; i < coreCommonGraphEdges.size(); i++) {
                const auto &e = coreCommonGraphEdges[i];
                commonGraph.add_edge({e.first, e.second, (e.first + e.second) % 16 + 1}, true);
            }
        );
        auto end = std::chrono::system_clock::now();
        fprintf(stderr, "Common Graph Initialization: %.6lfs\n", 1e-6*(uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(end-start).count());
    }
    fprintf(stderr, "Number of Edges in Common Graph is %lu\n", commonGraph.get_degree());
    
    uint64_t srcToCalculate = 100;
    
    double totalCommonAccuracy = 0.0;
    double totalUnionAccuracy = 0.0;
    
    for (uint64_t src = 0; src < srcToCalculate; src++) {
        uint64_t root = roots[src];
        
        auto originalResult = rootCompute(graph, root);
        auto commonResult = rootCompute(commonGraph, root);
        auto unionResult = rootCompute(unionGraph, root);
        
        uint64_t commonAccurateCount = 0;
        uint64_t unionAccurateCount = 0;
        
        for (uint64_t i = 0; i < graph.getNodesNum(); i++) {
            if (originalResult[i].data == commonResult[i].data) commonAccurateCount++;
            if (originalResult[i].data == unionResult[i].data) unionAccurateCount++;
        }
        
        totalCommonAccuracy += (commonAccurateCount * 100.0) / graph.getNodesNum();
        totalUnionAccuracy += (unionAccurateCount * 100.0) / graph.getNodesNum();
    }
    
    double averageCommonAccuracy = totalCommonAccuracy / srcToCalculate;
    double averageUnionAccuracy = totalUnionAccuracy / srcToCalculate;
    
    fprintf(stderr, "Average Accuracy Results:\n");
    fprintf(stderr, "  Number of Roots: %lu\n", srcToCalculate);
    fprintf(stderr, "  Average Common Graph Accuracy: %.2f%%\n", averageCommonAccuracy);
    fprintf(stderr, "  Average Union Graph Accuracy: %.2f%%\n", averageUnionAccuracy);
    fprintf(stderr, "----------------------------------------\n");
    return 0;
} 
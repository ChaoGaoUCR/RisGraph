...
auto rootCompute(Graph<uint64_t>& graph, uint64_t root) {
    ...
    auto update_func = [](uint64_t src, uint64_t dst, uint64_t src_data, uint64_t dst_data, AdjEdgeType adjedge) -> std::pair<bool, uint64_t> {
        uint64_t new_narrowness = std::max(src_data, adjedge.data); // 取最大值（路径最窄处最宽）
        return std::make_pair(new_narrowness < dst_data, new_narrowness);
    };
    ...
    auto init_label_func = [=](uint64_t vid) -> std::pair<uint64_t, bool> {
        return {vid == root ? 0 : UINT64_MAX, vid == root}; // 设 root 的狭窄度为 0，其他点为 ∞
    };
    ...
    THRESHOLD_OPENMP_LOCAL("omp parallel for", raw_edges_len, 1024,
        for (auto edge = 0; edge < raw_edges_len; edge++)
        {
            ...
            if (versionCheck(tag.first, tag.second, snapShotNum))
            {
                ...
                {
                    auto outRankSource = std::max(outRankResult[idx][src].data, edgeLen);
                    auto inRankSource = std::max(inRankResult[idx][dst].data, edgeLen);
                    auto outFlag = (outRankSource == outRankResult[idx][dst].data);
                    auto inFlag = (inRankSource == inRankResult[idx][src].data);                    
                    if (complex)
                    {
                        outFlag = (outRankSource == outRankResult[idx][dst].data);
                        inFlag = (inRankSource == inRankResult[idx][src].data);
                    }
                    else
                    {
                        outFlag = (outRankSource == outRankResult[idx][dst].data) && !outRankResultAtomic[idx][dst];
                        inFlag = (inRankSource == inRankResult[idx][src].data) && !inRankResultAtomic[idx][src];   
                    }

            ...
            if (snapshotGraph != 0)
            {
                ...
                for (auto & hotNode : rankTriple)
                {
                    ...
                    for (uint64_t anySrc = 0; anySrc < graph.getNodesNum(); anySrc++)
                    {
                        ...
                        auto lowerBound = std::min(hotToAnySrc, hotToTarget);
                        auto upperBound = std::max(hotToAnySrc, hotToTarget);
                        currentBoundaryAllocation[srcToTest][anySrc].first = lowerBound;
                        currentBoundaryAllocation[srcToTest][anySrc].second = upperBound;
